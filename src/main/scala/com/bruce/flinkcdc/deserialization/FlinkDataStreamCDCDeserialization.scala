...
/**
 * Description: 使用Flink DataStream时自定义反序列化数据格式
 * Date: 2024/10/25
 *
 * @author bruce
 */
object FlinkDataStreamCDCDeserialization {
  def main(args: Array[String]): Unit = {
    ...
    val mySqlSource = MySqlSource
      ...
      .deserializer(new MyCustomerDeserializationSchema())//指定数据反序列化类，可以将MySqlSource采集到的数据转换为指定格式
      .build()
    ...
    env.execute("FlinkDataStreamCDCDeserialization")
