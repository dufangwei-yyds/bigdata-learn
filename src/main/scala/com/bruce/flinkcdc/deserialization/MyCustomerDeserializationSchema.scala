package com.bruce.flinkcdc.deserialization
...
class MyCustomerDeserializationSchema extends DebeziumDeserializationSchema[String]{
  ...
  override def deserialize(sourceRecord: SourceRecord, collector: Collector[String]): Unit = {
    ...
    //SourceRecord{sourcePartition={server=mysql_binlog_source}, sourceOffset={transaction_id=null, ts_sec=1678332916, file=, pos=0}} ConnectRecord{topic='mysql_binlog_source.data.goods', kafkaPartition=null, key=Struct{id=3}, keySchema=Schema{mysql_binlog_source.data.goods.Key:STRUCT}, value=Struct{after=Struct{id=3,name=hammer,description=12oz carpenter's hammer},source=Struct{version=1.6.4.Final,connector=mysql,name=mysql_binlog_source,ts_ms=0,db=data,table=goods,server_id=0,file=,pos=0,row=0},op=r,ts_ms=1678332916193}, valueSchema=Schema{mysql_binlog_source.data.goods.Envelope:STRUCT}, timestamp=null, headers=ConnectHeaders(headers=)}
    ...
    //获取before
    //value=Struct{after=Struct{id=3,name=hammer,description=12oz carpenter's hammer}
